...
const { notionToken, databaseIdAngular } = require('../config');
...
async function getAngularTip(type = 'tip') {
    try {
        const response = await notion.databases.query({ database_id: databaseIdAngular });

        if (response.results.length === 0) {
            console.log(`⚠️ No hay ${type}s de Angular cargados en Notion.`);
            return null;
        }
        ...
        const tips = response.results.map(page => {
            ...
            return {
                ...
                theme: props.Theme?.select?.name || "Angular",
        ...
        if (availableTips.length > 0) {
            ...
            console.log(`⚠️ No hay ${type}s de Angular que cumplan exactamente las reglas. Seleccionando el mejor disponible...`);
        ...
        if (selectedTip) {
            ...
            console.log(`🅰️ ${type.charAt(0).toUpperCase() + type.slice(1)} de Angular seleccionado - Nivel: ${selectedTip.level}, Última aparición: ${selectedTip.lastAppearance || 'Primera vez'}`);
            return selectedTip;
        }

        console.log(`⚠️ No se pudo seleccionar un ${type} de Angular.`);
        return null;

    } catch (error) {
        console.error(`❌ Error al obtener el ${type} de Angular:`, error.message);
        return null;
    }
}
module.exports = { getAngularTip};